...
class comando_instalacion extends comando_toba
{
	 ...
	 *
	 * @consola_parametros Opcionales: [-d 'Id de Desarrollador'] [-t 'Tipo de instalación; 0=Desarrollo, 1=Producción'] [-h 'Profile/Host de PostgreSQL'] [-p 'Puerto de PostgreSQL'] [-u 'Usuario de PostgreSQL'] [-c 'Clave de PostgreSQL'] [-b 'Base de PostgreSQL'] [-k 'Clave inicial del usuario "toba"'].
	...
	function opcion__instalar()
	{
		...
		if (! $this->get_instalacion()->existe_base_datos_definida( $base ) ) {
			do {
				$profile = isset($param['-h']) ? $param['-h'] : $this->consola->dialogo_ingresar_texto( 'PostgreSQL - Ubicación (ENTER utilizará localhost)', false);
				if ($profile == ''){
					$profile = 'localhost';
				}
				$puerto_tmp = isset($param['-p']) ? $param['-p'] : $this->consola->dialogo_ingresar_texto( "PostgreSQL - Puerto (ENTER utilizará: $puerto)", false);
				if ($puerto_tmp != ''){
					$puerto = $puerto_tmp;
				}
				$usuario = isset($param['-u']) ? $param['-u'] : $this->consola->dialogo_ingresar_texto( 'PostgreSQL - Usuario (ENTER utilizará postgres)', false);
				if ($usuario == '') {
					$usuario = 'postgres';
				}
				$clave = isset($param['-c']) ? $param['-c'] : $this->consola->dialogo_ingresar_texto( 'PostgreSQL - Clave  (ENTER para usar sin clave)', false);
				$base_temp = isset($param['-b']) ? $param['-b'] : $this->consola->dialogo_ingresar_texto( "PostgreSQL - Base de datos (ENTER utilizará $base)", false);
				if ($base_temp != ''){
					$base = $base_temp;
				}
				if ($puerto_tmp != ''){
